    ...
    function getDetails(req, res, next, kind) {
        ...
        if (obj) {
            if (obj[kind]) {
                ...
                const ingredientKind = obj.foundation
                    ? "foundation"
                    : obj.extra
                    ? "extra"
                    : obj.protein
                ...
                res.status(StatusCodes.NOT_FOUND).send(
                    `Unknown ${kind}: ${req.params.name}. Did you mean /${ingredientKind}/${req.params.name}`
                );
            }
        } else {
            res.set("Content-Type", "text/plain");
            res.status(StatusCodes.NOT_FOUND).send(
                `Unknown ${kind}: ${req.params.name}`
            );
        }
    }
        ...
        try {
            if (!req.is("application/json")) {
                ...
                throw {
                    httpStatus: StatusCodes.BAD_REQUEST,
                    errorMessage:
                        "Type check faild. Type of body must be array, but is " +
                        typeof req.body,
                };
            } else {
                const order = {
            ...
            res.set("Content-Type", "text/plain");
            res.status(e?.httpStatus || StatusCodes.INTERNAL_SERVER_ERROR).send(
                e?.errorMessage ||
